...
function load_item(cate) { // cate는 그냥 걸어본 것. 그래도 한번 사용처를 고려해봐야겠다.
  // 설명: item 만들기
    if(ITEM_LIST[cate].length > 1) { // ITEM_LIST의 배열 [cate]의 길이가 1이 상일때 동작
      ...
      for(let j=0; j<ITEM_LIST[cate].length; j++){ // ITEM_LIST는 안의 배열 cate번째의 길이만큼 동작하게 한다. ex) top_ten은 ITEM_LIST의 0번째 배열이고 10개의 데이터를 가지고 있다.
  ...
  console.log('img의 height: ' + $(`#${EL_ARR[cate]} ul.item_box img`).height())
...
//   console.log(ITEM_LIST.slice(5, 7).length);
